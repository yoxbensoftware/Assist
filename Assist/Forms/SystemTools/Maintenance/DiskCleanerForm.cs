...
internal sealed class DiskCleanerForm : Form
{
    ...
    private async Task CleanAsync()
    {
        ...
        await Task.Run(() =>
        {
            foreach (var file in _filesToDelete)
            {
                ...
                {
                    // File in use or access denied ÔÇö skip
                }
            }
        });
        ...
        if (IsTargetChecked(3))
        {
            ...
            {
                EmptyRecycleBin();
                AppendLog("Geri d├Ân├╝┼ş├╝m kutusu bo┼şalt─▒ld─▒.");
            }
            catch
            {
                AppendLog("Geri d├Ân├╝┼ş├╝m kutusu bo┼şalt─▒lamad─▒.");
            }
        }

        AppendLog($"\r\nTemizlik tamamland─▒: {deleted} dosya silindi, {FormatSize(freedBytes)} bo┼şalt─▒ld─▒.");
        _lblStatus.Text = $"Temizlik tamamland─▒: {deleted}/{_filesToDelete.Count} dosya silindi ({FormatSize(freedBytes)})";
    ...
    private void ScanDirectory(string path, string label)
    {
        ...
        AppendLog($"[{label}] Taran─▒yor: {path}");
        ...
        {
            AppendLog($"  ÔÜá Eri┼şim hatas─▒: {path}");
        }
    ...
    {
        AppendLog("[Log Dosyalar─▒] Taran─▒yor...");
        var count = 0;
        ...
        AppendLog($"  Bulunan: {count} log dosyas─▒ (7+ g├╝n eski)");
    }

    private void ScanThumbnailCache()
    {
        AppendLog("[Thumbnail Cache] Taran─▒yor...");
        ...
        AppendLog($"  Bulunan: {count} thumbnail cache dosyas─▒");
    }
